...
For this section I will use some dependently typed pseudo code which should
hopefully be straightforward to translate into Agda, also by looking at the
[Examples](./src/Examples/) folder.

The sorts of `disp(TT)` are:
...
meta level (`Set`), we could also write
...
Sometimes we want to move between the indexed universe `(Setᵇ, Setᶠ)` of
`disp(TT)` and a non-indexed universe `Set`. In Agda specifically, it would be
nice to be able to embed native Agda types (like `data` and `record`
definitions) inside the base and fiber fragments. Luckily, we can do this by
adding the following equivalence to our SOGAT (where `≃` again is
...
fracture : Set ≃ (Aᵇ : Setᵇ) × Setᶠ Aᵇ
```

This is currently partially implemented, and I am planning to add more notes on
it soon. The suggestion to do this was by Trebor Huang.
