...
--
-- This indexed type theory is separated by a proposition # in the sense that
-- the base is #-modal (λ _ b → b : base → (# → base) is an equivalence) and
-- the fibers are #-contracible (# → fiber b ≃ ⊤). Technically we don't need to
-- include # here (we can just use the base/fiber separation) but the intended
-- models support it.
--
-- We ignore size issues for now..

module over (# : Prop) where
  ...
  variable
    ...
    p : Prop
    Aᵇ# Bᵇ# Cᵇ# : # → Tyᵇ
    Fᵇ# Gᵇ# Hᵇ# : (p : #) → Tmᵇ (Aᵇ# p) → Tyᵇ
    aᵇ# bᵇ# cᵇ# : (p : #) → Tmᵇ (Aᵇ# p)
    fᵇ# gᵇ# hᵇ# : (p : #) → (xᵇ : Tmᵇ (Aᵇ# p)) → Tmᵇ (Fᵇ# p xᵇ)
